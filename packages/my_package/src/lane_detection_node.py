...
class LaneDetectionNode(DTROS):
    ...
    def detect_lane(self, image):
        ...
        # Define an ROI. For example, use the bottom third of the image.
        roi_y_start = int(height * 2/3)
        roi_x_start, roi_x_end = int(width/10), int(width-(width/10))
        ...
        if lines is not None:
            for line in lines:
                x1, y1, x2, y2 = line[0]
                # Classify lines into left and right halves based on the ROI’s width
                if x1 < width // 2 and x2 < width // 2:
                    left_x.extend([x1, x2])
                elif x1 > width // 2 and x2 > width // 2:
                    right_x.extend([x1, x2])
                # Note: y values are within the ROI. Save them.
                top_y.extend([y1, y2])
                bottom_y.extend([y1, y2])
        
        if left_x and right_x and top_y and bottom_y:
            # Use ROI coordinates to compute the bounding box for the lane in the ROI
            min_x, max_x = max(left_x), min(right_x)
            min_y, max_y = min(top_y), max(bottom_y)

            focal_length = 650*0.1
            # Compute the center of the full image
            cx, cy = width // 2, height // 2
            
            # Calculate approximate distances (you may need to adjust these computations)
            Z_min = focal_length / (max_y)
            Z_max = focal_length / (max_y - (cy - roi_y_start))
            ...
            # Adjust the y coordinates to the full image by adding the roi_y_start offset.
            cv2.rectangle(image, (min_x, min_y + roi_y_start), (max_x, max_y + roi_y_start), (0, 255, 0), 3)
            cv2.putText(image, f"Lane Length: {lane_length_meters:.2f} m", (min_x, min_y + roi_y_start - 10),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
            cv2.putText(image, f"Lane Width: {lane_width_meters:.2f} m", (min_x, min_y + roi_y_start - 30),
                        cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)

...
class DShapeNode(DTROS):
    def __init__(self, node_name):
        ...
        # Topic names for subscribers (subscribers initialized later)
        ...
        # Control parameters
        self.KP = 0.015  # Proportional gain for lane following
        self.TARGET_DISTANCE = 1.35  # meters
    
        signal.signal(signal.SIGINT, self.signal_handler)

    def setup(self):
        """Initialize subscribers after the object is fully constructed."""
    ...
    def cb_left_encoder(self, msg):
        ...
        if not hasattr(self, 'last_left_ticks') or self.last_left_ticks is None:
    ...
    def cb_right_encoder(self, msg):
        current_ticks = msg.data
        # Guard against uninitialized attributes
        if not hasattr(self, 'last_right_ticks') or self.last_right_ticks is None:
    ...
    def turn_off_leds(self):
        ...
        pattern = LEDPattern()
        off_color = ColorRGBA(0, 0, 0, 1)  # Fully transparent/off
        pattern.color_list = ['OFF'] * 5
        pattern.rgb_vals = [off_color] * 5
        pattern.color_mask = [1] * 5
        pattern.frequency = 0.0  # No blinking
    ...
    def set_led_color(self, color):
        ...
        colors = {
            ...
            'PURPLE': ColorRGBA(0.5, 0, 0.5, 1)  # Added purple color
        }
    ...
    def lane_follow(self):
        ...
        while not rospy.is_shutdown():
            ...
            if avg_distance >= self.TARGET_DISTANCE:
                self.stop()
                rospy.loginfo("Target distance reached, setting LEDs to purple...")
                self.set_led_color('PURPLE')  # Set LEDs to purple
                rospy.sleep(1.0)  # Wait for 1 second to show purple
                rospy.loginfo("Turning off LEDs...")
                self.turn_off_leds()  # Turn off LEDs
                break
    ...
    def run(self):
        # Setup subscribers after initialization
        self.setup()
        rospy.sleep(0.5)  # Small delay to ensure everything is ready
        self.lane_follow()

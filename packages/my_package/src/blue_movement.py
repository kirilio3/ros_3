...
class DShapeNode(DTROS):
    ...
    def cb_camera(self, msg):
        ...
        processed_image, blue_distance = self.detect_blue_line(blurred_image)
        
        if blue_distance is None:
            rospy.loginfo("Blue line reached, stopping the robot.")
            return
    ...
    def detect_blue_line(self, image):
        ...
        lower_blue = np.array([100, 150, 50], np.uint8)   # Lower bound for darker blue
        upper_blue = np.array([140, 255, 255], np.uint8) # Upper bound for darker blue
        ...
        if contours:
            ...
            if y + h < height and x + w < width and x > 0 and y > 0:
                ...
                if pixel_height > 0:
                    ...
                    cv2.putText(image, f"Blye Line Distance: {distance:.2f} m", (x, y - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.6, (0, 255, 0), 2)
    ...
    def straight_line(self, distance):
        ...
        while not rospy.is_shutdown() and not self.blue_line_reached:
    ...
    def move_arc_quarter_odometry(self, radius, speed, clockwise=False):
        ...
        twist.v = speed
        twist.omega = omega
        self.set_led_color('ORANGE')  # Set LED color to CYAN when rotating
        ...
        while not rospy.is_shutdown():
            ...
            rospy.loginfo("Delta Heading:")
            rospy.loginfo(delta_heading)
            rospy.loginfo("Info:")
            rospy.loginfo(-((target_change-self.TOL_ANGLE) - self.TOL))
            if clockwise:
    ...
    def run(self):
        ...
        rospy.sleep(3.0)
        self.move_arc_quarter_odometry(0.08, 0.7, True)
        rospy.sleep(2.0)

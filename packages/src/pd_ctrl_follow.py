...
class DShapeNode(DTROS):
    def __init__(self, node_name):
        ...
        self.KD = 0.006   # Derivative gain (new for PD controller)
    ...
    def lane_follow(self):
        rospy.loginfo("Starting lane following with PD control...")
        ...
        while not rospy.is_shutdown():
            ...
            if yellow_msg is not None and white_msg is not None:
                ...
                rospy.loginfo(f"Err der: {error_derivative}")
                rospy.loginfo(f"Err : {error}")
                # PD control signal
                ...
                # If lanes not detected, turn slightly while moving forward

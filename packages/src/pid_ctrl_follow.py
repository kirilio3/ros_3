...
class DShapeNode(DTROS):
    def __init__(self, node_name):
        ...
        # Control parameters
        self.KP = 0.022
        # self.KP = 0.0225  # Proportional gain for lane following
        # self.KP = 0.027  # Proportional gain
        self.KD = 0.0105
        
        # self.KI = 0.00005  # Integral gain (new for PID controller)
        self.KI = 0.001
    ...
    def lane_follow(self):
        ...
        while not rospy.is_shutdown():
            ...
            if yellow_msg is not None and white_msg is not None:
                ...
                # Conditional velocity adjustment based on error magnitude
                if abs(error) > 10 and abs(error) < 30:
                    omega = max(min(omega, self.OMEGA_SPEED), -self.OMEGA_SPEED)
                ...
                # if abs(error) > 40 and abs(error) < 65:
                #     omega = max(min(omega, self.OMEGA_SPEED), -self.OMEGA_SPEED)
                #     rospy.loginfo(omega)
                #     cmd = Twist2DStamped(v=self.VELOCITY//2, omega=omega//1.5)
                # else:
                #     omega = max(min(omega, self.OMEGA_SPEED), -self.OMEGA_SPEED)
                #     cmd = Twist2DStamped(v=self.VELOCITY//3, omega=omega//2)
